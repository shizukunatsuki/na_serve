...
- **逐级收尾**：收尾先对整组发 `SIGTERM`。半秒后还没退，几乎总是在等一个没传完的下载
  ...
  信号后就不再自己处理信号，`SIGINT` 回到后台任务启动时的"忽略"，只有再来一个 `SIGTERM`
  才能结束它。对仍然无视这些信号的子进程，5 秒宽限后升级为 `SIGKILL`。
...
- 单实例、多实例并发；
- 各种中断路径：`Ctrl-C`、`Ctrl-\`、只对 `serve` 本身发 `TERM`/`HUP`/`USR1` 等终止信号、
  关终端、父 shell 被杀（包括不是会话首进程、内核不会补发 `SIGHUP` 的嵌套 shell）、服务
  自身崩溃、输出管道的读端提前退出、停止时还有下载没传完（加 `--real-tunnel` 时经由
  真实隧道下载）；
